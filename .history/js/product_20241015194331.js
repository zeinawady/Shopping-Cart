...
function addToCart(id) {
    ...
    itemDiv.innerHTML = `
        <div class="name me-2">${choosenItem.title}</div>
        <div class="price me-2">$${choosenItem.price}</div>
        <div class="trashIcon" style="cursor: pointer;"><i class="fa-solid fa-trash"></i></div>
    `;
    ...
    trashIcon.addEventListener("click", function() {
        ...
        total -= choosenItem.price; // Subtract the price of the removed item
        count--; // Decrease the count
        
        // Update the display
        sum.innerHTML = count > 0 ? "Total Price = $" + total : "No Products Added";
        circleSum.innerHTML = count; // Update the item count
    });
