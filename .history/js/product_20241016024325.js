...
function loadCart(){
    ...
    if(cart.length>0){
        cart.forEach(item => {
            ...
            itemDiv.classList.add('item', 'd-flex', 'justify-content-between', 'align-items-center', 'my-4');
            itemDiv.innerHTML = `
        <div class="name me-2">${item.title}</div>
        <div class="price me-2">$${item.price}</div>
        <div class="trashIcon" style="cursor: pointer;" onclick="deleteItem(${item.id},this,${item.price})"><i class=" fa-solid fa-trash"></i></div>
    `;
...
logoutIcon.addEventListener('click', function(){
    ...
    logoutDiv.classList.add('m-auto' , 'bg-success' ,'py-5', 'px-5')
    setTimeout(function(){
