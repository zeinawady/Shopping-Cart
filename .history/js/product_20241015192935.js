...
function addToCart(id) {
    ...
    let itemDiv=` <div class="item d-flex justify-content-between align-items-center my-4">
                   
                   
                </div>`

   item.innerHTML+=` <div class="name me-2">${choosenItem.title}</div>
                    <div class="price me-2">$${choosenItem.price}</div>
                    <div class="trashIcon"><i class="fa-solid fa-trash" style="cursor: pointer;"></i></div>
        `
    listCart.innerHTML += itemDiv;
    total += choosenItem.price;
    count++;

    sum.innerHTML = "Total Price= $" + total;
    circleSum.innerHTML = count;
let trashIcon=itemDiv.querySelector('.trashIcon');
